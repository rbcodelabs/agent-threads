...
This resolves the open question of whether Orchestrator keeps the Voice plugin id: it does not. The migration path is the compatibility obligation created by that choice, and it must tolerate being run against an already-migrated profile, a missing legacy file, and a profile that already carries its own settings.
