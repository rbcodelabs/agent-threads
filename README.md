...
![Agent Threads](https://img.shields.io/badge/Obsidian-Plugin-7C3AED) ![Version](https://img.shields.io/badge/version-0.42.0-blue) [![Roadmap](https://img.shields.io/badge/Roadmap-Compass-6366F1)](https://compass.rbcodelabs.com/portal/rbcodelabs/claude-threads/roadmap)
...
**Codex computer use** is off by default in **Settings → Agent harness**. Off requests denied default app access and disables Codex's bundled computer-use plugins and known desktop-capable MCP entry points, including the legacy `node_repl` server. This can also remove the shared Codex browser REPL; Agent Threads' own browser tools are unaffected. On inherits your local Codex configuration, without installing tools or overriding its access restrictions. Changes apply when sessions next start or restart, including resumed threads. Existing running and idle sessions keep their current access; reload the plugin after active work finishes to apply the choice to all sessions. This setting does not restrict arbitrary third-party automation tools or replace OS permissions.
...
Geode/Obsidian peer plugins can discover the generation-scoped Agent Threads API at `app.plugins.plugins['claude-threads']?.api?.v1`. It provides immutable thread operations, sanitized cursor-based trace projection, fail-closed constrained Claude runs for input-only evaluation, host-confirmed MCP server and secret registration, and a contribution surface. `extensions.registerArtifactProvider` lets a plugin present its own durable artifact on a thread through a declarative contract — it returns a title and named actions, and never receives a view, a workspace leaf, or a DOM node. The `artifacts` namespace is the matching entry point: a peer can attach an artifact to a thread and invoke one of its actions with no view access at all, and a host-validated `storageRoot` gives artifact files a lifecycle, so deleting a thread cleans them up. `extensions.registerAgentTool` goes further and lets a plugin contribute an in-process agent tool that every thread session can call: the peer writes a thread-agnostic `invoke`, and the host binds it per thread and injects the calling thread id, so a peer never touches the per-thread MCP server factory. Names are checked against host built-ins, core agent tools and other peers, so a contribution can never shadow `Read` or `Bash`; contributed tools require approval unless they opt out, and a tool that throws or hangs degrades to a single tool error rather than breaking the session. Two supporting reads make that usable: `threads.permissions` exposes the effective permission mode and pending-plan state, and `artifacts.allocateStorage` creates and returns a host-owned artifact directory instead of making peers guess the vault layout. Built-in Design mode creates, previews *and registers its `EnterDesignMode` tool* through that same public path. `capabilities` is computed from what the host can actually do, so check it before calling. See [Peer Plugin API v1](docs/public-api.md) and the checked-in [type contract](api/public-api-v1.d.ts).
